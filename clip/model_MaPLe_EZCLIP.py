...
class Transformer(nn.Module):
    def __init__(
        ...
        self.model_for = model_for
        self.config = config
        val = math.sqrt(6.0 / float(3 * reduce(mul, (16, 16), 1) + self.width))
        if self.model_for == "image":
            ...
            self.resblocks = nn.Sequential(
                *[
                    ResidualAttentionBlock_MaPLe(
                        ...
                        i,
                        dropout=dropout[i],
                        frames=config.data.num_segments,
                        model_for=model_for,
                    )
                    for i in range(layers)
                ]
            ...
            self.resblocks = nn.Sequential(
                *[
                    ResidualAttentionBlock(
                        config, width, heads, attn_mask, model_for=model_for
                    )
...
class VisualTransformer_MaPLe(nn.Module):
    def __init__(
        self,
        config,
    ...
    def forward(
        ...
        if not self.first_layer:
            if len(compound_prompts_deeper) > 0:
                ...
                if not self.text_layer and self.model_for == "image":
                    #!Added: Multi-modality prompt
...
class CLIP(nn.Module):
    def __init__(
        ...
        #!New##############################################
        if trainer == "MaPLe":
            self.visual = VisualTransformer_MaPLe(
                config,
