...
def load_clip_to_cpu(cfg):
    ...
    model_path = cfg.network.pretrained_model
...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        classnames = [name.replace("_", " ") for name in classnames] # [dog, cat,..., something else]
        name_lens = [len(_tokenizer.encode(name)) for name in classnames] # [1,1,...,2]
        prompts = [prompt_prefix + " " + name + "." for name in classnames]# ['X X X...X dog', 'X X X...X cat', ]

        tokenized_prompts = torch.cat([clip.tokenize(p) for p in prompts])# [n_cls, n_tkn, d_model]
        with torch.no_grad():
            embedding = clip_model.token_embedding(tokenized_prompts).type(dtype) # [n_cls, n_tks, transformer.width], n_tks-> [SOS] + n_ctx + [CLS] [EOS]
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        self.dtype = clip_model.dtype
        print(f"self.dtype = {self.dtype}") #  torch.float16
    ...
    def create_logits(self, image_features, text_features):
        image_features = image_features / image_features.norm(dim=-1, keepdim=True)
        text_features = text_features / text_features.norm(dim=-1, keepdim=True)
...
def main():
    ...
    for epoch in range(start_epoch, config.solver.epochs):
        ...
        for kkk, (prompt_images, list_id) in enumerate((train_loader)):
            ...
            '''
            logits_per_image, logits_per_text = customCLIP.module.create_logits(
        ...
        if is_best:
            ...
            best_saving(working_dir, epoch, model, optimizer, config.data.dataset)
