...
if __name__ == '__main__':
    train_path = r"/root/wj/EZ_CLIP/AFE/DVS128Gesture/DVS128Gesture_train.txt" # TODO: Change to your directory
    val_path = r"/root/wj/EZ_CLIP/AFE/DVS128Gesture/DVS128Gesture_val.txt" # TODO: Change to your directory
    whole_path = r"/root/wj/EZ_CLIP/AFE/DVS128Gesture/DVS128Gesture_whole.txt" 
    tf = open("/root/wj/EZ_CLIP/AFE/DVS128Gesture/DVS128Gesture.json", "r") # TODO: Change to your directory
    ...
    datasets = DVS128Gesture(whole_path, representation='rgb', augmentation=False)
    ...
    for step, (events_image, class_idxs, event_stream_path) in enumerate(feeder):
        ...
        for m in range(len(events_image)):
            ...
            for i in range(B):
                for j in range(T):
                    # TODO train
                    file_path = event_stream_path[0].replace('DVS128Gesture', 'DVS128Gesture_Sampled_EZCLIP').replace('.aedat', '')
                    # TODO val
                    # file_path = event_stream_path[0].replace('DvsGesture', 'DVSGesture_Sampled_val').replace('.aedat','')

                    folder_path = '/'.join([file_path.split('/')[i] for i in range(4)]) #! changed
                    ...
                    file_path = class_path + '/' + file_path.split('/')[-1] #! changed
                    if not os.path.exists(file_path):
                        os.makedirs(file_path)
                    subfile_path = file_path + '/' + 'img_{:05d}.jpg'.format(j+1) #! changed
                    print(subfile_path)
