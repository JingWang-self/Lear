...
class SeACT(Dataset):
    def __init__(self, txtPath, num_events=100000, median_length=100000,
                 ...
                 augmentation=False, pad_frame_255=False, min_frame_num=8):  # Added min_frame_num
        ...
        self.min_frame_num = min_frame_num  # Store min_frame_num
    ...
    def __getitem__(self, idx):
        ...
        label_str = event_stream_path.split('/')[5].split('-')[0][-4:]
    ...
    def generate_event_image(self, events_clip, shape, representation):
        ...
        # print(f'event_frame.shape:{event_frame.shape}') # (224,224,3)
        return event_frame
...
if __name__ == '__main__':
    all_path = r'/root/autodl-tmp/SeAct.txt' # TODO: Change to your directory
    num_events = 100000 # 1093470 546735 273367 136683 68341
    median_length = 1500000
    frame = 1513
    tf = open("/root/wj/ExACT/Dataloader/SeAct/SeAct.json", "r") # TODO: Change to your directory
    ...
    for step, (events_image, class_idxs, event_stream_path) in enumerate(feeder):
        ...
        for i in range(B):
            for j in range(T):
                file_path = event_stream_path[0].replace('SeAct', 'SeAct_Sampled_EZCLIP').replace('.aedat', '') #! changed:'SeAct_Sampled_EZCLIP'
                folder_path = '/'.join([file_path.split('/')[i] for i in range(5)]) #! 7->5
                # print(file_path)
                if not os.path.exists(folder_path):
                    os.makedirs(folder_path)
                class_path = '/'.join([folder_path, file_path.split('/')[5].split('-')[0][-4:]]) #! changed
                # print(class_path)
                if not os.path.exists(class_path):
                    os.makedirs(class_path)
                file_path = class_path + '/' + 'img_{:05d}.jpg'.format(j+1) #! changed: name
                print(file_path)
