...
if __name__ == '__main__':
    ...
    for step, (events_image, class_idxs, event_stream_path) in enumerate(feeder):
        ...
        for i in range(B):
            for j in range(T):
                file_path = event_stream_path[0].replace('SeAct', 'SeAct_Sampled_8').replace('.aedat', '')
                folder_path = '/'.join([file_path.split('/')[i] for i in range(5)])
                # print(file_path)
                if not os.path.exists(folder_path):
                    os.makedirs(folder_path)
                class_path = '/'.join([folder_path, file_path.split('/')[5].split('-')[0][-4:]])
                # print(class_path)
