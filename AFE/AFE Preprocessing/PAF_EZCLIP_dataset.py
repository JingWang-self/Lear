...
class PAF(Dataset):
    def __init__(self, txtPath, num_events=100000, median_length=100000,
                 ...
                 augmentation=False, pad_frame_255=False, classnames_dict=None, min_frame_num=8):
    ...
    def __getitem__(self, idx):
        ...
        # label
        label_str = event_stream_path.split('/')[4] #! changed
...
if __name__ == '__main__':
    all_path = r'/root/wj/EZ_CLIP/AFE/PAF/PAF_whole_45.txt' # TODO: Change to your directory
    num_events = 80000 # 1093470 546735 273367 136683 68341
    median_length = 1500000
    frame = 1513
    tf = open("/root/wj/EZ_CLIP/AFE/PAF/PAF.json", "r")
    ...
    datasets = PAF(all_path, representation='rgb', median_length = median_length,
                   num_events = num_events, frame=frame, augmentation=False, classnames_dict=classnames_dict)
    ...
    for step, (events_image, class_idxs, event_stream_path) in enumerate(feeder):
        ...
        for i in range(B):
            for j in range(T):
                file_path = event_stream_path[0].replace('PAF', 'PAF_Sampled_EZCLIP').replace('.aedat', '') #!changed
                folder_path = '/'.join([file_path.split('/')[i] for i in range(5)]) # TODO
                # print(file_path)
                if not os.path.exists(folder_path):
                    os.makedirs(folder_path)
                class_path = '/'.join([folder_path, file_path.split('/')[5]]) # TODO
                # print(class_path)
                if not os.path.exists(class_path):
                    os.makedirs(class_path)
                file_path = class_path + '/' + 'img_{:05d}.jpg'.format(j+1) #! changed
                print(file_path)
